...
function Subproblem(instance::Instance)
    ...
    reduced_cost = zeros(Float64, n, n)

    ng = zeros(Int, delta - 1, n)

    tbounds = [Inf for _ in 1:128, _ in 1:n, _ in 1:tmax(instance)]

    return Subproblem(instance, pi, reduced_cost, ng, 0.0, tbounds)
end
...
@inline reduced_cost(sp::Subproblem, i::Int, j::Int) = @inbounds sp.reduced_cost[i, j]
@inline adjacent(sp::Subproblem, i::Int, j::Int) = @inbounds sp.reduced_cost[i, j] < Inf

@inline backward_reduced_cost(sp::Subproblem, i::Int, j::Int) = @inbounds sp.reduced_cost[j, i]
...
function set(sp::Subproblem, constraints::Vector{ConstraintRef})
    ...
    for i in 1:n
        for j in 1:i-1
            sp.reduced_cost[i, j] = distance(sp, i, j) - sp.pi[i] / 2.0 - sp.pi[j] / 2.0
        end

        sp.reduced_cost[i, i] = Inf
...
function build_ng(sp::Subproblem)
    ...
    for i in 2:n
        ...
        sort!(adj, by=x -> adjacent(sp, i, x) ? sp.reduced_cost[i, x] : Inf)
...
@inline set_branching_0(sp::Subproblem, u::Int, v::Int) = sp.reduced_cost[u, v] = Inf
...
function set_branching_1(sp::Subproblem, u::Int, v::Int)
    for i in 1:n_nodes(sp)
        ...
        sp.reduced_cost[u, i] = Inf
        sp.reduced_cost[i, v] = Inf
    end
end
...
function compute_rc(sp::Subproblem, route::Vector{Int})
    ...
    for i in 1:length(route)-1
        rc += reduced_cost(sp, route[i], route[i+1])
    end
